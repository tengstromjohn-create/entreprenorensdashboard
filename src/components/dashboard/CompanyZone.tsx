  ...
  if (!loading && !companyData && !profile?.org_number) {
    return (
      <div className="space-y-6">
        <h1 className="text-xl font-bold text-[#1F2A30]">Mitt bolag</h1>
        <div className="bg-[#F5F5F0] rounded-lg p-8 text-center">
          <Rocket size={40} className="text-gray-400 mx-auto mb-4" />
          <h2 className="text-lg font-semibold text-[#1F2A30] mb-2">Har du inte bildat bolag än?</h2>
          ...
          <button
            ...
            className="bg-[#1F2A30] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#2F3A40] transition-colors inline-flex items-center gap-2"
          >
      ...
      {companyData?.boardMembers && companyData.boardMembers.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-100 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-[#1F2A30]">Styrelse</h3>
          </div>
          <div className="space-y-2">
            {companyData.boardMembers.map((member, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-[#1F2A30]">{member.name}</span>
